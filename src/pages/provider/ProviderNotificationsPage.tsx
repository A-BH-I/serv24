...
import {
  ...
  FileCheck, Briefcase, Star, Wallet, ShieldCheck, AlertCircle, Info, ExternalLink
} from 'lucide-react';
...
function getNotificationIcon(type: string) {
  if (type.includes('document') || type.includes('verification')) return { icon: FileCheck, color: 'text-primary', bg: 'bg-primary/10' };
  if (type.includes('booking') || type.includes('job')) return { icon: Briefcase, color: 'text-emerald-500', bg: 'bg-emerald-50' };
  if (type.includes('review') || type.includes('rating')) return { icon: Star, color: 'text-amber-500', bg: 'bg-amber-50' };
  if (type.includes('payment') || type.includes('wallet') || type.includes('earning')) return { icon: Wallet, color: 'text-violet-500', bg: 'bg-violet-50' };
  if (type.includes('approved') || type.includes('verified')) return { icon: ShieldCheck, color: 'text-primary', bg: 'bg-primary/10' };
  if (type.includes('rejected') || type.includes('suspended')) return { icon: AlertCircle, color: 'text-destructive', bg: 'bg-destructive/10' };
...
function matchesFilter(n: Notification, filter: FilterKey): boolean {
  ...
  if (filter === 'document') return t.includes('document') || t.includes('verification') || t.includes('verified') || t.includes('rejected');
  if (filter === 'booking') return t.includes('booking') || t.includes('job');
  if (filter === 'payment') return t.includes('payment') || t.includes('wallet') || t.includes('earning') || t.includes('withdrawal');
  return true;
}
...
export default function ProviderNotificationsPage() {
  ...
  const handleTap = (n: Notification) => {
    ...
    const link = getNotificationLink(n, 'provider');
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-2">
            {filtered.map((n, i) => {
              ...
              const hasLink = !!getNotificationLink(n, 'provider');
              return (
                <div
                  key={n.id}
                  onClick={() => handleTap(n)}
