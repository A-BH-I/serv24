...
import wrenchesImg from '@/assets/maintenance-wrenches.png';
...
export default function MaintenancePage() {
  ...
  const headline = settings.banner_maintenance_title || "We'll Be Back";
  const tagline =
    settings.banner_maintenance_subtitle ||
    'Our workshop is currently being upgraded for a better experience. Maintenance mode is in effect. We will notify you when we reopen.';
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSubmitting(true);
    const res = await subscribersApi.subscribe(email.trim(), 'maintenance');
    setSubmitting(false);
    if (res.success) {
      setDone(true);
      setEmail('');
    ...
    >
      ...
      <section className="relative z-10 mx-auto flex min-h-[100svh] max-w-3xl flex-col items-center justify-center px-5 py-10 text-center">
        ...
        <img
          src={wrenchesImg}
          alt=""
          className="my-5 sm:my-7 h-28 sm:h-36 w-auto object-contain drop-shadow-md"
          loading="lazy"
        />

        <p className="max-w-xl text-sm sm:text-base text-slate-600 [text-wrap:pretty] leading-relaxed">
          {tagline}
        </p>

        {settings.banner_maintenance_until && <Countdown until={settings.banner_maintenance_until} />}
